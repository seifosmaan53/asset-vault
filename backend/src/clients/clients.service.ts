...
export class ClientsService {
  ...
  async findAll(
    ...
    this.logger.log(
      `[DIAGNOSTIC] ClientsService.findAll called with actorUserId: ${actorUserId}`,
    );
    ...
    if (filters?.createdAtFrom) {
      queryBuilder.andWhere('client.createdAt >= :createdAtFrom', {
        createdAtFrom: filters.createdAtFrom,
      });
    }
    if (filters?.createdAtTo) {
      queryBuilder.andWhere('client.createdAt <= :createdAtTo', {
        createdAtTo: filters.createdAtTo,
      });
    }
    if (filters?.updatedAtFrom) {
      queryBuilder.andWhere('client.updatedAt >= :updatedAtFrom', {
        updatedAtFrom: filters.updatedAtFrom,
      });
    }
    if (filters?.updatedAtTo) {
      queryBuilder.andWhere('client.updatedAt <= :updatedAtTo', {
    ...
    this.logger.log(
      `[DIAGNOSTIC] ClientsService.findAll returned ${clients.length} clients for userId: ${actorUserId}`,
    );
    if (clients.length > 0) {
      this.logger.log(
        `[DIAGNOSTIC] First client: id=${clients[0].id}, name=${clients[0].name}, userId=${clients[0].userId}`,
      );
    } else {
      ...
      const allClients = await this.clientsRepository
        ...
        .getMany();
      this.logger.warn(
        `[DIAGNOSTIC] No active clients found, but found ${allClients.length} total clients (including deleted) for userId: ${actorUserId}`,
      );
      if (allClients.length > 0) {
        this.logger.warn(
          `[DIAGNOSTIC] Sample client: id=${allClients[0].id}, userId=${allClients[0].userId}, deletedAt=${allClients[0].deletedAt}`,
        );
      }
    }

    this.logger.debug(
      `findAll: Returning ${clients.length} clients (excluding soft-deleted)`,
    );
    return clients;
  }
  ...
  async findOne(id: string, actorUserId: string): Promise<Client> {
    ...
    if (anyClient.deletedAt) {
      this.logger.warn(
        `Client ${id} is soft-deleted (deletedAt=${anyClient.deletedAt})`,
      );
    ...
    if (!client) {
      ...
      this.logger.error(
        `Client ${id} was found but query failed - this should not happen`,
      );
      throw new NotFoundException('Client not found');
    }
  ...
  async create(actorUserId: string, data: Partial<Client>): Promise<Client> {
    ...
    const sanitizedData: Partial<Client> = {
      ...
      addressJson: data.addressJson
        ? {
            street: data.addressJson.street
              ? sanitizeString(data.addressJson.street, 255)
              : data.addressJson.street,
            city: data.addressJson.city
              ? sanitizeString(data.addressJson.city, 100)
              : data.addressJson.city,
            state: data.addressJson.state
              ? sanitizeString(data.addressJson.state, 100)
              : data.addressJson.state,
            zip: data.addressJson.zip
              ? sanitizeString(data.addressJson.zip, 20)
              : data.addressJson.zip,
            country: data.addressJson.country
              ? sanitizeString(data.addressJson.country, 100)
              : data.addressJson.country,
          }
        : data.addressJson,
    };
  ...
  async update(
    ...
    if (data.notes !== undefined)
      sanitizedData.notes = data.notes
        ? sanitizeString(data.notes, 5000)
        : data.notes;
  ...
  async remove(id: string, actorUserId: string): Promise<void> {
    this.logger.debug(
      `remove: Starting deletion for client ${id}, actorUserId=${actorUserId}`,
    );
    ...
    try {
      ...
      if (anyClient.deletedAt) {
        ...
        this.logger.debug(
          `remove: Client ${id} is already soft-deleted (deletedAt=${anyClient.deletedAt}), returning early`,
        );
      ...
      if (!client) {
        ...
        this.logger.debug(
          `remove: Client ${id} not found with lock - may have been deleted concurrently`,
        );
      ...
      if (client.deletedAt) {
        ...
        this.logger.debug(
          `remove: Client ${id} is already soft-deleted (deletedAt=${client.deletedAt}), returning early`,
        );
        return; // Idempotent - already deleted, success
      }
      ...
      if (activeInvoices > 0) {
        ...
        this.logger.warn(
          `remove: Cannot delete client ${id}: has ${activeInvoices} active invoice(s)`,
        );
      ...
      if (activeStores > 0) {
        ...
        this.logger.warn(
          `remove: Cannot delete client ${id}: has ${activeStores} active store(s)`,
        );
        throw new BadRequestException(
      ...
      if (updateResult.affected === 0) {
        this.logger.error(
          `remove: CRITICAL - UPDATE query affected 0 rows for client ${id}!`,
        );
        if (transactionStarted) {
          await queryRunner.rollbackTransaction();
        }
        throw new Error(
          'Failed to soft-delete client: UPDATE query did not affect any rows',
        );
      }

      this.logger.debug(
        `remove: UPDATE query executed for client ${id}, affected rows: ${updateResult.affected}`,
      );
      ...
      if (!verifyDeleted) {
        this.logger.error(
          `remove: CRITICAL - Client ${id} not found after UPDATE!`,
        );
        if (transactionStarted) {
          await queryRunner.rollbackTransaction();
        }
        throw new Error(
          'Failed to soft-delete client: client not found after deletion',
        );
      }

      if (!verifyDeleted.deletedAt) {
        this.logger.error(
          `remove: CRITICAL - Client ${id} was not actually soft-deleted! deletedAt is null after UPDATE`,
        );
        if (transactionStarted) {
          await queryRunner.rollbackTransaction();
        }
      ...
      this.logger.debug(
        `remove: Verified client ${id} has deletedAt=${verifyDeleted.deletedAt} in database before commit`,
      );
      ...
      this.logger.debug(
        `remove: Client ${id} soft-deleted successfully using direct UPDATE query`,
      );
      ...
      setImmediate(async () => {
        try {
          ...
          if (stillExists) {
            this.logger.warn(
              `remove: Client ${id} was soft-deleted but still appears in findAll results. This may be due to connection pooling or replication lag.`,
            );
          } else {
            this.logger.debug(
              `remove: Verified - Client ${id} confirmed removed from findAll results`,
            );
          }
        } catch (verifyError) {
          // Don't fail deletion if verification fails - it's just for logging
          this.logger.warn(
            `remove: Could not verify deletion via findAll`,
            verifyError,
          );
        }
      });
      ...
      if (transactionStarted) {
        try {
          ...
          this.logger.error(
            `remove: Error rolling back transaction for client ${id}`,
            rollbackError,
          );
        }
      }
      ...
      if (error.code === '23503') {
        ...
        throw new NotFoundException(
          'Cannot delete client: has associated records',
        );
      }
      ...
      try {
        ...
        this.logger.error(
          `remove: Error releasing query runner for client ${id}`,
          releaseError,
        );
      }
    }
  ...
  async bulkRemove(
    ...
    this.logger.debug(
      `bulkRemove: Starting bulk deletion for ${ids.length} clients, actorUserId=${actorUserId}`,
    );
    ...
    for (const id of ids) {
      try {
        ...
        this.logger.warn(
          `bulkRemove: Failed to delete client ${id}: ${error.message}`,
        );
        failed.push({
          id,
  ...
  async importFromFile(
    ...
    this.logger.debug(
      `importFromFile: Starting import for user ${actorUserId}`,
    );
      ...
      for (const row of batch) {
        try {
          ...
          const clientData: Partial<Client> = {
            ...
            notes:
              row.notes || row.Notes || row['Additional Notes'] || undefined,
            addressJson: {
