...
export class SampleDataController {
  ...
  @ApiOperation({
    ...
    description:
      "Adds sample clients, inventory items, and invoices to the authenticated user's account",
  })
  @ApiResponse({
    status: 201,
  ...
  async addSampleData(@Request() req: AuthenticatedRequest) {
    try {
      const result = await this.sampleDataService.addSampleData(
        req.user.userId,
      );
      return {
  ...
  @ApiOperation({
    ...
    description:
      'Deletes sample clients, stores, inventory items, and invoices that were created by the sample data feature',
  })
  @ApiResponse({
    status: 200,
    ...
    schema: {
      ...
      properties: {
        message: {
          type: 'string',
          example: 'Sample data deleted successfully',
        },
        data: {
          type: 'object',
          properties: {
  ...
  async deleteSampleData(@Request() req: AuthenticatedRequest) {
    const result = await this.sampleDataService.deleteSampleData(
      req.user.userId,
    );
    return {
