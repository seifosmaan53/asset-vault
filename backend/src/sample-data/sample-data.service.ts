...
export class SampleDataService {
  ...
  private generateClientData(): Array<{
    ...
    const notes = [
      'Regular customer, pays on time',
      'Bulk orders, quarterly billing',
      'New client, growing business',
    ...
    for (let i = 0; i < 50; i++) {
      ...
      clients.push({
        ...
        tags: [tags[i % tags.length], tags[(i + 1) % tags.length]].filter(
  ...
  private generateInventoryData(): Array<Partial<InventoryItem>> {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      const defaultUnitPrice =
        Math.round(costPrice * (1.5 + Math.random() * 1.5) * 100) / 100;
      ...
      const averageWeeklyUsage =
        Math.round((currentStock / (weeksSupplyTargetOverride * 4)) * 100) /
        100;
  ...
  async addSampleData(userId: string): Promise<{
    clients: number;
    stores: number;
    inventory: number;
    invoices: number;
    ...
    this.logger.log(
      `Adding comprehensive sample data for user: ${user.email} (${userId})`,
    );
    ...
    const existingClients = await this.clientRepository
      ...
      .andWhere(
        '(client.name IN (:...names) OR client.email IN (:...emails))',
        { names: clientNames, emails: clientEmails },
      )
      .getMany();
    ...
    existingClients.forEach((client) => {
      ...
      if (client.email)
        existingClientMap.set(client.email.toLowerCase(), client);
    });
    ...
    for (const data of clientData) {
      ...
      const existing =
        existingClientMap.get(nameKey) ||
        (emailKey ? existingClientMap.get(emailKey) : undefined);
    ...
    this.logger.log(
      `Created ${clients.length} clients (${clients.length - existingClients.length} new, ${existingClients.length} existing)`,
    );
    ...
    const existingStoreCodeSet = new Set(
      allExistingStores
        .map((store) => store.code?.toLowerCase())
        .filter(Boolean),
    );
    ...
    for (const data of storeData) {
      ...
      if (
        existingStoreCodeSet.has(codeLower) ||
        usedCodesInBatch.has(codeLower)
      ) {
        // If it's an active store in database, reuse it (only if not already in batch)
        const existing = activeStores.find(
          (s) => s.code?.toLowerCase() === codeLower,
        );
        ...
        while (
          existingStoreCodeSet.has(uniqueCode.toLowerCase()) ||
          usedCodesInBatch.has(uniqueCode.toLowerCase())
        ) {
          counter++;
      ...
      if (
        usedCodesInBatch.has(finalCodeLower) ||
        existingStoreCodeSet.has(finalCodeLower)
      ) {
    ...
    this.logger.log(
      `Created ${stores.length} stores (${newStoresCount} new, ${reusedStoresCount} existing)`,
    );
    ...
    this.logger.log(
      `Created ${inventoryItems.length} inventory items (${newInventoryCount} new, ${reusedInventoryCount} existing)`,
    );
    ...
    this.logger.log(
      `Sample data summary: ${result.clients} clients, ${result.stores} stores, ${result.inventory} inventory items, ${result.invoices} invoices`,
    );

    return result;
  }

  async deleteSampleData(userId: string): Promise<{
    deleted: {
      clients: number;
      stores: number;
      inventory: number;
    ...
    const escapedCompanies = sampleCompanies.map((c) =>
      c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
    );
    const escapedTypes = sampleTypes.map((t) =>
      t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
    );
    ...
    const sampleStorePattern =
      /^(MAI|DOW|UPT|WES|EAS|NOR|SOU|CEN|EXP|PRE|OUT|WAR|DIS|RET|FLA|HUB|DEP|FAC|LOC|BRA)-\d{3}(-\d+)?$/i;
    ...
    const sampleInventoryPattern =
      /^(ITEM-\d{4}|PROD-\d{3}|SERV-\d{3}|SRV-\d{3}|WID-\d{3}|PRD-\d{3})$/i;
    ...
    const sampleClientEmailPattern =
      /^contact\d{3}@[a-z]+(corp|inc|llc|ltd|co|group|solutions|services|systems|industries)\.com$/i;
    ...
    const seedClientPattern = new RegExp(
      `^(${seedClientNames.map((n) => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})$`,
      'i',
    );
    ...
    const sampleClients = allClients.filter((c) => {
      ...
      // Check email pattern
      const emailMatches = c.email
        ? sampleClientEmailPattern.test(c.email)
        : false;
      ...
      if (!matches && c.name) {
        ...
        this.logger.log(
          `Client "${c.name}" (email: ${c.email}) does not match sample pattern`,
        );
      }
      return matches;
    });
    ...
    const sampleInventory = allInventory.filter((i) => {
      ...
      if (!matches) {
        ...
        this.logger.debug(
          `Inventory "${i.sku}" (name: ${i.name}) does not match sample pattern`,
        );
      }
      return matches;
    });
    ...
    this.logger.log(
      `Found ${sampleClients.length} sample clients out of ${allClients.length} total`,
    );
    this.logger.log(
      `Found ${sampleStores.length} sample stores out of ${allStores.length} total`,
    );
    this.logger.log(
      `Found ${sampleInventory.length} sample inventory items out of ${allInventory.length} total`,
    );
    ...
    const invoicesToDeleteByClient =
      sampleClientIds.length > 0
        ? await this.invoiceRepository
            ...
            .andWhere('invoice.clientId IN (:...clientIds)', {
              clientIds: sampleClientIds,
            })
            .getMany()
        : [];

    const invoicesToDeleteByStore =
      sampleStoreIds.length > 0
        ? await this.invoiceRepository
            .createQueryBuilder('invoice')
            .withDeleted() // Include soft-deleted invoices
            .where(
            ...
            .andWhere('invoice.storeId IN (:...storeIds)', {
              storeIds: sampleStoreIds,
            })
            .getMany()
        : [];

    // Get invoices that reference sample inventory items via invoice items
    const invoicesToDeleteByInventory =
      sampleInventoryIds.length > 0
        ? await this.invoiceRepository
            .createQueryBuilder('invoice')
            ...
            .andWhere('item.inventoryItemId IN (:...inventoryIds)', {
              inventoryIds: sampleInventoryIds,
            })
            .getMany()
        : [];
    ...
    if (invoicesToDelete.length > 0) {
      ...
      this.logger.log(
        `Deleted ${deletedInvoices} invoices (hard delete, including soft-deleted). Breakdown: ${invoicesToDeleteByClient.length} by client, ${invoicesToDeleteByStore.length} by store, ${invoicesToDeleteByInventory.length} by inventory`,
      );
    }
    ...
    if (sampleInventory.length > 0) {
      ...
      this.logger.log(
        `Deleted ${deletedInventory} inventory items (hard delete)`,
      );
    }
    ...
    if (remainingClients > 0 || remainingInventory > 0) {
      this.logger.warn(
        `Warning: ${remainingClients} clients and ${remainingInventory} inventory items were not deleted (may not be sample data)`,
      );

      // Log the actual names/SKUs that weren't deleted to help debug
      if (remainingClients > 0) {
        const notDeletedClients = allClients.filter(
          (c) => !sampleClients.includes(c),
        );
        const clientNames = notDeletedClients
          .map((c) => `"${c.name}" (${c.email || 'no email'})`)
          .join(', ');
        this.logger.warn(
          `Clients not deleted (first 10): ${clientNames.substring(0, 500)}`,
        );
      }
      if (remainingInventory > 0) {
        const notDeletedInventory = allInventory.filter(
          (i) => !sampleInventory.includes(i),
        );
        const inventoryList = notDeletedInventory
          .slice(0, 10)
          .map((i) => `"${i.sku}" (${i.name?.substring(0, 30) || 'no name'})`)
          .join(', ');
