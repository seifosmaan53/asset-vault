...
export class QuotaGuard implements CanActivate {
  ...
  private metricToFeatureKey(metric: UsageMetric): string {
    ...
    const base = metric.replace(/_created$/, '');
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const skipQuota = this.reflector.getAllAndOverride<boolean>(
      SKIP_QUOTA_KEY,
    ...
    const isDuplication =
      ...
      (request.body &&
        (request.body.isDuplicate === true ||
          request.body.isDuplicate === 'true'));

    if (skipQuota || isDuplication) {
      this.logger.debug(
        `Skipping quota check for duplication: query=${request.query?.duplicate}, header=${request.headers['x-duplicate']}, body=${request.body?.isDuplicate}`,
      );
      return true;
    }
    ...
    const metric =
      this.reflector.get<UsageMetric>('quota-metric', context.getHandler()) ||
    ...
    // Log for debugging
    this.logger.debug(
      `Checking quota: metric=${metric}, featureKey=${featureKey}, limit=${limit}`,
    );
    ...
    if (!hasQuota) {
      this.logger.warn(
        `Quota exceeded for user ${user.userId}, metric: ${metric}`,
      );
