...
export class StripeWebhookController {
  ...
  async handleWebhook(
    ...
    if (!webhookSecret || webhookSecret.trim() === '') {
      this.logger.error(
        'STRIPE_WEBHOOK_SECRET not set - webhook verification cannot proceed',
      );
  ...
  private async handleCheckoutSessionCompleted(
    ...
    try {
      ...
      if (session.mode === 'subscription' && session.subscription) {
        ...
        if (!userId) {
          this.logger.warn(
            `No userId in checkout session metadata: ${session.id}`,
          );
          return;
        }
        ...
        this.logger.log(
          `Stripe subscription status: ${stripeSubscription.status} for subscription ${subscriptionId}`,
        );
        ...
        this.logger.log(
          `Successfully updated subscription for user ${userId} to status: ${updatedSubscription.status}`,
        );
      } else {
        this.logger.warn(
          `Checkout session ${session.id} is not a subscription session or missing subscription`,
        );
      }
    } catch (error: any) {
      this.logger.error(
        `Error handling checkout session completed for ${session.id}:`,
        error,
      );
  ...
  private async handleSubscriptionCreated(
    ...
    try {
      ...
      if (!userId) {
        this.logger.warn(
          `No userId in subscription metadata: ${subscription.id}`,
        );
        return;
      }
      ...
      this.logger.error(
        `Error handling subscription created for ${subscription.id}:`,
        error,
      );
      throw error; // Re-throw to be caught by main handler
    }
  ...
  private async handleSubscriptionUpdated(
    ...
    try {
      ...
      this.logger.error(
        `Error handling subscription updated for ${subscription.id}:`,
        error,
      );
      throw error; // Re-throw to be caught by main handler
    }
  ...
  private async handleSubscriptionDeleted(
    ...
    try {
      ...
      this.logger.error(
        `Error handling subscription deleted for ${subscription.id}:`,
        error,
      );
      throw error; // Re-throw to be caught by main handler
    }
  ...
  private async handleInvoicePaymentSucceeded(
    ...
    try {
      ...
      this.logger.error(
        `Error handling invoice payment succeeded for ${invoice.id}:`,
        error,
      );
      throw error; // Re-throw to be caught by main handler
    }
  ...
  private async handleInvoicePaymentFailed(
    ...
    try {
      ...
      this.logger.error(
        `Error handling invoice payment failed for ${invoice.id}:`,
        error,
      );
      throw error; // Re-throw to be caught by main handler
    }
