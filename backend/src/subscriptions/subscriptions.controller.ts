...
export class SubscriptionsController {
  ...
  async createCheckoutSession(@Req() req: AuthenticatedRequest) {
    ...
    if (!customerId) {
      ...
      if (subscription && !subscription.stripeCustomerId) {
        await this.subscriptionsService.updateSubscriptionCustomerId(
          req.user.userId,
          customerId,
        );
  ...
  @ApiOperation({
    ...
    description: 'Get current usage metrics for the authenticated user',
  })
  ...
  async getBillingHistory(@Req() req: AuthenticatedRequest) {
    const invoices = await this.subscriptionsService.getBillingHistory(
      req.user.userId,
    );
    return {
  ...
  @ApiOperation({
    ...
    description:
      'Verify a Stripe checkout session and sync subscription status',
  })
  @ApiResponse({
    status: 200,
    description: 'Checkout session verified successfully',
  })
  @ApiResponse({ status: 400, description: 'Bad request' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiOperation({
    ...
    description:
      'Manually sync subscription status from Stripe (useful when webhooks are not working)',
  })
  @ApiResponse({ status: 200, description: 'Subscription synced successfully' })
  @ApiResponse({
    status: 200,
    description:
      'No subscription to sync - user has no active Stripe subscription',
  })
  async syncSubscription(@Req() req: AuthenticatedRequest) {
    try {
      ...
      if (!subscription.stripeSubscriptionId) {
        ...
        this.logger.debug(
          `User ${req.user.userId} has subscription but no Stripe subscription ID yet`,
        );
        return {
          message:
            'Subscription found but no Stripe subscription ID. Please complete checkout first.',
          subscription: {
            id: subscription.id,
            status: subscription.status,
            plan: subscription.plan,
