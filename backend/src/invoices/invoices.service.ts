...
import {
  ...
  PaginatedResponse,
} from '../common/utils/pagination-links.util';
...
} from './utils/invoice-totals.util';
...
export class InvoicesService {
  ...
  async findAll(
    ...
    try {
      ...
      if (filters?.storeId) {
        query.andWhere('invoice.storeId = :storeId', {
      ...
      if (filters?.issueDateFrom) {
        query.andWhere('invoice.issueDate >= :issueDateFrom', {
          issueDateFrom: filters.issueDateFrom,
        });
      }
      if (filters?.issueDateTo) {
        query.andWhere('invoice.issueDate <= :issueDateTo', {
      ...
      if (filters?.paidDateFrom) {
        query.andWhere('invoice.paidAt >= :paidDateFrom', {
          paidDateFrom: filters.paidDateFrom,
        });
      }
      if (filters?.paidDateTo) {
        query.andWhere('invoice.paidAt <= :paidDateTo', {
          paidDateTo: filters.paidDateTo,
        });
      }
      if (filters?.createdAtFrom) {
        query.andWhere('invoice.createdAt >= :createdAtFrom', {
          createdAtFrom: filters.createdAtFrom,
        });
      }
      if (filters?.createdAtTo) {
        query.andWhere('invoice.createdAt <= :createdAtTo', {
      ...
      if (filters?.totalMin !== undefined) {
        query.andWhere('invoice.total >= :totalMin', {
          totalMin: filters.totalMin,
        });
      }
      if (filters?.totalMax !== undefined) {
        query.andWhere('invoice.total <= :totalMax', {
          totalMax: filters.totalMax,
        });
      }
      if (filters?.subtotalMin !== undefined) {
        query.andWhere('invoice.subtotal >= :subtotalMin', {
          subtotalMin: filters.subtotalMin,
        });
      }
      if (filters?.subtotalMax !== undefined) {
        query.andWhere('invoice.subtotal <= :subtotalMax', {
          subtotalMax: filters.subtotalMax,
        });
      }
      ...
      this.logger.log(
        `[DIAGNOSTIC] findAll returned ${results.length} invoices for userId: ${userId}`,
      );
      if (results.length > 0) {
        this.logger.log(
          `[DIAGNOSTIC] First invoice: id=${results[0].id}, number=${results[0].number}, userId=${results[0].userId}`,
        );
      } else {
        ...
        const allInvoices = await this.invoicesRepository
          ...
          .getMany();
        this.logger.warn(
          `[DIAGNOSTIC] No active invoices found, but found ${allInvoices.length} total invoices (including deleted) for userId: ${userId}`,
        );
        if (allInvoices.length > 0) {
          this.logger.warn(
            `[DIAGNOSTIC] Sample invoice: id=${allInvoices[0].id}, userId=${allInvoices[0].userId}, deletedAt=${allInvoices[0].deletedAt}`,
          );
        }
      }
      ...
      this.logger.error(
        `[findAll] Error fetching invoices for userId ${userId}: ${error.message}`,
        error.stack,
      );
  ...
  async findPaged(
    ...
    if (params?.subtotalMin !== undefined) {
      query.andWhere('invoice.subtotal >= :subtotalMin', {
        subtotalMin: params.subtotalMin,
      });
    }
    if (params?.subtotalMax !== undefined) {
      query.andWhere('invoice.subtotal <= :subtotalMax', {
        subtotalMax: params.subtotalMax,
      });
    }
  ...
  async findOne(id: string, userId: string): Promise<Invoice> {
    ...
    const cacheKey = buildCacheKey(CacheKey.INVOICE_DETAIL, {
      invoiceId: id,
      userId,
    });
  ...
  async create(
    ...
    if (
      ...
      try {
        ...
        throw new BadRequestException(
          `Store with ID "${data.storeId}" not found or does not belong to user`,
        );
      }
      ...
      if (data.items && data.items.length > 0) {
        const operation =
          (data.status || 'draft') === 'draft' ? 'reserve' : 'sale';
        try {
    ...
    const validatedCurrency = validateCurrencyCode(
      data.currency || 'USD',
      'currency',
    );
    ...
    const invoiceData = {
      ...
      status: (data.status || 'draft') as
        ...
        | 'paid'
        | 'overdue'
        | 'cancelled', // Default to 'draft' if not provided
      issueDate: issueDate,
      dueDate: dueDate,
    ...
    if (
      invoiceData.metadataJson?.terms &&
      typeof invoiceData.metadataJson.terms === 'string'
    ) {
      invoiceData.metadataJson.terms = sanitizeString(
        invoiceData.metadataJson.terms,
        5000,
      );
    }
    ...
    const notesValue =
      typeof invoiceData.notes === 'string' ? invoiceData.notes : undefined;
    ...
    try {
      ...
      if (!invoice.number) {
        invoice.number = await this.generateInvoiceNumber(
          userId,
          invoice.type,
          settings,
          queryRunner,
      ...
      this.logger.log(
        `Invoice created: ${savedInvoice.number} (${savedInvoice.id})`,
      );
          ...
          if (item.inventoryItemId) {
            if (typeof item.inventoryItemId === 'string') {
              ...
              if (
                ...
                this.logger.warn(
                  `Invalid inventoryItemId format: ${trimmed}, setting to null`,
                );
              }
            } else {
              this.logger.warn(
                `Invalid inventoryItemId type: ${typeof item.inventoryItemId}, expected string, setting to null`,
              );
            }
          }
          ...
          return {
            ...
            taxRate:
              item.taxRate !== undefined && item.taxRate !== null
                ? Number(Number(item.taxRate).toFixed(2))
                : 0,
            discountRate:
              item.discountRate !== undefined && item.discountRate !== null
                ? Number(Number(item.discountRate).toFixed(2))
                : 0,
      ...
      itemsToInsert.forEach((item, index) => {
        this.logger.log(
          `Item ${index} before insert - lineTotal: ${item.lineTotal}, type: ${typeof item.lineTotal}, quantity: ${item.quantity}, unitPrice: ${item.unitPrice}`,
        );
        if (
          item.lineTotal === null ||
          ...
          this.logger.error(
            `CRITICAL: Item ${index} has invalid lineTotal before insert: ${item.lineTotal}`,
          );
          // Force recalculate
          ...
          item.lineTotal = Number(
            (Math.round((afterDisc + taxAmount) * 100) / 100).toFixed(2),
          );
          this.logger.log(
            `Recalculated lineTotal for item ${index}: ${item.lineTotal}`,
          );
        }
      });
      ...
      this.logger.log(
        `Inserting ${itemsToInsert.length} items with lineTotal values: ${itemsToInsert.map((i) => i.lineTotal).join(', ')}`,
      );
      ...
      if (itemsToInsert.length > 0) {
        ...
        values.forEach((val, idx) => {
          ...
          if (isNaN(val.quantity) || val.quantity < 1) {
            throw new BadRequestException(
              `Invalid quantity at index ${idx}: ${val.quantity}. Must be a positive integer.`,
            );
          }
          ...
          if (
            val.lineTotal === null ||
            val.lineTotal === undefined ||
            isNaN(val.lineTotal)
          ) {
            this.logger.error(
              `CRITICAL: Value ${idx} has invalid lineTotal: ${val.lineTotal}, fixing...`,
            );
            val.lineTotal = 0;
          }
          this.logger.log(
            `Item ${idx} - lineTotal: ${val.lineTotal} (${typeof val.lineTotal}), quantity: ${val.quantity} (${typeof val.quantity}), unitPrice: ${val.unitPrice}`,
          );
        });
        ...
        for (const val of values) {
          ...
          if (
            finalLineTotal === null ||
            finalLineTotal === undefined ||
            isNaN(finalLineTotal)
          ) {
            ...
            this.logger.warn(
              `Final safety check: Recalculated lineTotal from ${val.lineTotal} to ${finalLineTotal}`,
            );
          }
          ...
          if (
            ...
            finalLineTotal = 0;
            this.logger.error(
              `CRITICAL: lineTotal still invalid after all checks, defaulting to 0`,
            );
          }
          ...
          this.logger.log(
            `Inserting item with lineTotal: ${finalLineTotal} (type: ${typeof finalLineTotal})`,
          );
      ...
      try {
        ...
        this.logger.error(
          `Failed to track usage for invoice creation: ${error.message}`,
        );
      }
    ...
    try {
      ...
      const invoiceWithRelations = await queryRunner.manager
        ...
        .leftJoinAndSelect(
          ...
          'client',
          'client.deletedAt IS NULL',
        ) // CRITICAL: Exclude soft-deleted clients
        .leftJoinAndSelect('invoice.store', 'store', 'store.deletedAt IS NULL') // CRITICAL: Exclude soft-deleted stores
      ...
      if (data.status !== undefined) {
        ...
        if (data.status !== oldStatus) {
          ...
          this.logger.log(
            `Invoice status transition: ${oldStatus} -> ${data.status}`,
          );
          ...
          const statusHistory = queryRunner.manager.create(
      ...
      if (data.type !== undefined && data.type !== oldType) {
        ...
        if (oldStatus === 'paid' && data.type === 'estimate') {
          throw new BadRequestException(
            'Cannot change paid invoice to estimate',
          );
        }
        ...
        if (
          ...
          throw new BadRequestException(
            'Cannot change paid estimate to invoice',
          );
        }
      }
      ...
      if (data.clientId !== undefined) {
        ...
        if (data.clientId !== invoice.clientId) {
          ...
          if (!client) {
            throw new NotFoundException(
              `Client with ID "${data.clientId}" not found or does not belong to organization`,
            );
          }
        }
      }
      ...
      if (validatedStoreId) {
        try {
          ...
          throw new BadRequestException(
            `Store with ID "${validatedStoreId}" not found or does not belong to user`,
          );
        }
        ...
        if (
          ...
          itemsToValidate.some((item: any) => item.inventoryItemId)
        ) {
          const operation =
            (data.status !== undefined ? data.status : invoice.status) ===
            'draft'
              ? 'reserve'
              : 'sale';
      ...
      if (data.status !== undefined) {
        ...
        this.logger.log(
          `Status update requested: ${invoice.status} -> ${data.status}`,
        );
      }
      if (data.notes !== undefined) {
        updateData.notes =
          typeof data.notes === 'string'
            ? sanitizeString(data.notes, 10000)
            : undefined;
      }
      ...
      if ((data as any).metadataJson?.terms !== undefined) {
        updateData.metadataJson = {
          ...
          terms: (data as any).metadataJson.terms
            ? sanitizeString((data as any).metadataJson.terms, 5000)
            : (data as any).metadataJson.terms,
        };
      }
      ...
      if (data.items) {
        ...
        const createdItems = items.map((item) =>
          queryRunner.manager.create(InvoiceItem, item as Partial<InvoiceItem>),
        );
        ...
        if (itemsWithoutIds.length > 0) {
          this.logger.error(
            `Some invoice items were saved without IDs: ${itemsWithoutIds.length} items`,
          );
          throw new BadRequestException(
            'Failed to save invoice items. Transaction will be rolled back.',
          );
        }
        ...
        const validationTotalsResult = computeInvoiceTotalsCents(
          savedItemsForValidation,
        );
        ...
        const totalDiff = Math.abs(
          updateData.total - validationTotals.invoice.total,
        );
        if (totalDiff > 0.01) {
          this.logger.error(
            `Total mismatch: calculated=${updateData.total}, validated=${validationTotals.invoice.total}`,
          );
          throw new BadRequestException(
            'Invoice totals validation failed. Transaction will be rolled back.',
          );
        }

        this.logger.log(
          `Saved ${savedItems.length} invoice items for invoice ${id}`,
        );
        delete data.items;
      }
      ...
      if (
        ...
        throw new BadRequestException(
          'Cannot change status from cancelled. Please create a new invoice.',
        );
      }
      ...
      if (data.status !== undefined) {
        ...
        if (data.status !== 'paid' && invoice.status === 'paid') {
          ...
          if (data.paidAt === null || data.paidAt === undefined) {
            updateData.paidAt = undefined;
            this.logger.log(
              `Clearing paidAt for invoice ${invoice.number} (status changed from paid to ${data.status})`,
            );
          }
        }
      }
      ...
      this.logger.log(
        `Updating invoice ${id} with data: ${JSON.stringify(updateData)}`,
      );

      // Ensure status is explicitly included if it's being updated
      if (data.status !== undefined && !updateData.status) {
        this.logger.warn(
          `Status was in data but not in updateData! Adding it explicitly.`,
        );
        updateData.status = data.status;
      }
      ...
      if (shouldGenerateInvoiceNumber) {
        const settings = await this.userSettingsService.getSettings(userId);
        updateData.number = await this.generateInvoiceNumber(
          userId,
          'invoice',
          settings,
          queryRunner,
        ...
        this.logger.log(
          `Generated new invoice number within transaction: ${updateData.number}`,
        );
      }
      ...
      if (updateData.clientId) {
        ...
        if (!clientExists) {
          throw new BadRequestException(
            `Client with ID "${updateData.clientId}" does not exist or does not belong to your organization.`,
          );
        }
      }
      ...
      if (updateData.storeId) {
        ...
        if (!storeExists) {
          throw new BadRequestException(
            `Store with ID "${updateData.storeId}" does not exist or does not belong to your organization.`,
          );
        }
      }
      ...
      if (invoiceWithItems.items && invoiceWithItems.items.length > 0) {
        this.logger.log(
          `Invoice ${id} has ${invoiceWithItems.items.length} items after update`,
        );
        invoiceWithItems.items.forEach((item, index) => {
          this.logger.log(
            `Item ${index}: id=${item.id}, inventoryItemId=${item.inventoryItemId}, quantity=${item.quantity}`,
          );
        });
      } else {
      ...
      if (
        data.status !== undefined &&
        invoiceWithItems.status !== data.status
      ) {
        this.logger.error(
          `Status update failed! Expected ${data.status} but got ${invoiceWithItems.status}`,
        );
        throw new BadRequestException(
          `Failed to update invoice status. Expected ${data.status} but got ${invoiceWithItems.status}`,
        );
      }

      // CRITICAL FIX #5, #9, #23, #38: Validate stock before creating new movements
      const newStatus =
        data.status !== undefined ? data.status : invoiceWithItems.status;
      const newType =
        data.type !== undefined ? data.type : invoiceWithItems.type;
      const newItems = invoiceWithItems.items || [];
      const newStoreId =
        data.storeId !== undefined ? data.storeId : invoiceWithItems.storeId;
      ...
      if (needsStockUpdate && newItems.length > 0) {
        ...
        if (newStoreId) {
          ...
          if (!store) {
            throw new BadRequestException(
              `Store with ID "${newStoreId}" not found or does not belong to your organization.`,
            );
          }
        }

        // CRITICAL FIX #5: Validate stock availability before creating movements
        const itemsNeedingStock = newItems.filter(
        ...
        for (const newItem of itemsNeedingStock) {
          ...
          if (!inventoryItem) {
            throw new NotFoundException(
              `Inventory item ${newItem.inventoryItemId} not found`,
            );
          }
          ...
          if (isNaN(quantityValue) || quantityValue <= 0) {
            throw new BadRequestException(
              `Invalid quantity: ${newItem.quantity}. Quantity must be a positive number.`,
            );
          }
          const quantity = Math.floor(quantityValue); // Ensure integer for database column type 'int'
          if (quantity < 1) {
            throw new BadRequestException(
              `Quantity must be at least 1 (got ${quantityValue} which rounded to ${quantity})`,
            );
          }
          ...
          if (newStoreId) {
            try {
              const stockValidation =
                await this.storeStockValidator.validateStoreStockAvailability(
                  newStoreId,
          ...
          if (!newItem.id) {
            this.logger.error(
              `Invoice item missing ID for invoice ${id}, inventoryItemId: ${newItem.inventoryItemId}`,
            );
            throw new BadRequestException(
              'Invoice item is missing an ID. Please try updating the invoice again.',
            );
          }
          ...
          const invoiceItemExists = await queryRunner.manager.findOne(
            InvoiceItem,
            {
              where: {
                id: newItem.id,
          ...
          if (!invoiceItemExists) {
            this.logger.error(
              `Invoice item ${newItem.id} not found in database for invoice ${id}. Item data: ${JSON.stringify({ id: newItem.id, invoiceId: id, inventoryItemId: newItem.inventoryItemId, description: newItem.description })}`,
            );
            throw new BadRequestException(
              `Invoice item ${newItem.id} not found. The invoice item may not have been saved properly. Please try updating the invoice again.`,
            );
          }

          // CRITICAL FIX: Check if stock movement already exists for this invoice item to prevent duplicates
          const existingMovement = await queryRunner.manager.findOne(
            StockMovement,
            {
              where: {
          ...
          if (existingMovement) {
            this.logger.log(
              `Stock movement already exists for invoice item ${newItem.id}, skipping creation`,
            );
          ...
          try {
            ...
            this.logger.log(
              `Created stock movement for invoice ${id}, item ${newItem.id}, quantity ${newItem.quantity}`,
            );
          } catch (movementError: any) {
            // Log detailed error information for debugging
            this.logger.error(
              `Failed to create movement for invoice ${id}, item ${newItem.id}:`,
              movementError,
            );
            this.logger.error(
              `Movement data: ${JSON.stringify({ inventoryItemId: newItem.inventoryItemId, invoiceItemId: newItem.id, storeId: newStoreId, quantity: newItem.quantity })}`,
            );
      ...
      // Organizations removed - cache keys no longer include organizationId
      const detailCacheKey = buildCacheKey(CacheKey.INVOICE_DETAIL, {
        invoiceId: id,
        userId,
      });
      const statsCacheKey = buildCacheKey(CacheKey.INVOICE_STATS, { userId });
      ...
      const newAffectsStock =
        ...
        newStatus !== 'cancelled' &&
        newStatus !== 'draft';
      const stockWasAffected =
        (oldAffectsStock && oldItems.length > 0) ||
        (newAffectsStock && newItems.length > 0);
      ...
      if (stockWasAffected) {
        ...
        if (storeIdToCheck) {
          cacheKeysToInvalidate.push(
            // Organizations removed - cache keys no longer include organizationId
            buildCacheKey(CacheKey.STORE_STOCK, {
      ...
      if (error instanceof Error) {
        ...
        if (
          ...
          this.logger.error(
            `Deadlock detected during invoice update for ${id}, transaction rolled back:`,
            error,
          );
          throw new BadRequestException(
            'A database deadlock occurred. Please try again.',
          );
        }
        ...
        if (isConstraintError) {
          ...
          this.logger.error(
            `Constraint violation during invoice update for ${id}:`,
            error,
          );
          this.logger.error(`Error message: ${error.message}`);
          this.logger.error(`Error code: ${(error as any)?.code || 'unknown'}`);
          this.logger.error(
            `Error detail: ${(error as any)?.detail || 'none'}`,
          );
          this.logger.error(
            `Error constraint: ${(error as any)?.constraint || 'none'}`,
          );
          this.logger.error(`Error table: ${(error as any)?.table || 'none'}`);
          this.logger.error(
            `Error column: ${(error as any)?.column || 'none'}`,
          );
          this.logger.error(
          ...
          let specificError =
            'Invalid data: constraint violation. Please check all referenced entities exist.';
          ...
          const allErrorText =
            `${fullErrorMsg} ${errorDetail} ${errorConstraint} ${errorTable} ${errorColumn}`.toLowerCase();
          ...
          if (
            allErrorText.includes('stock_movements') ||
            allErrorText.includes('stock_movement')
          ) {
            // Check if it's a store-related constraint in stock_movements
            if (
              allErrorText.includes('store') ||
              allErrorText.includes('store_id') ||
              errorColumn === 'storeId'
            ) {
              specificError =
                'Invalid store ID. The store does not exist or does not belong to your organization.';
            } else if (
              allErrorText.includes('inventory') ||
              allErrorText.includes('inventory_item') ||
              errorColumn === 'inventoryItemId'
            ) {
              specificError =
                'Invalid inventory item ID. One or more inventory items do not exist or do not belong to your organization.';
            } else if (
              allErrorText.includes('invoice_item') ||
              errorColumn === 'invoiceItemId'
            ) {
              specificError =
                'Invalid invoice item ID. The invoice item does not exist or is invalid.';
            } else {
              specificError =
                'Invalid data: constraint violation in stock movement. Please check all referenced entities (store, inventory item, invoice item) exist.';
            }
          } else if (
            allErrorText.includes('client') ||
            allErrorText.includes('clients') ||
            errorColumn === 'clientId'
          ) {
            specificError =
              'Invalid client ID. The client does not exist or does not belong to your organization.';
          } else if (
            allErrorText.includes('store') ||
            allErrorText.includes('stores') ||
            errorColumn === 'storeId'
          ) {
            specificError =
              'Invalid store ID. The store does not exist or does not belong to your organization.';
            ...
            allErrorText.includes('inventory') ||
            allErrorText.includes('inventory_items') ||
            allErrorText.includes('invoice_items')
          ) {
            specificError =
              'Invalid inventory item ID. One or more inventory items do not exist or do not belong to your organization.';
          } else {
        ...
        if (
          ...
          this.logger.error(
            `Unique constraint violation during invoice update for ${id}:`,
            error,
          );
          throw new BadRequestException(
            'Duplicate entry detected. This invoice number may already exist.',
          );
        }
      }

      this.logger.error(
        `Invoice update failed for ${id}, transaction rolled back:`,
        error,
      );
      throw error;
    } finally {
  ...
  async remove(id: string, userId: string): Promise<void> {
    ...
    try {
      ...
      if (!anyInvoice) {
        this.logger.warn(
          `Invoice deletion attempted for non-existent invoice: ${id} by user ${userId}`,
        );
        await queryRunner.rollbackTransaction();
        await queryRunner.release();
      ...
      if (anyInvoice.deletedAt) {
        this.logger.debug(
          `Invoice ${id} is already soft-deleted (deletedAt: ${anyInvoice.deletedAt}), returning early (idempotent)`,
        );
        await queryRunner.rollbackTransaction();
        await queryRunner.release();
      ...
      if (anyInvoice.userId !== userId) {
        this.logger.warn(
          `Invoice deletion attempted for invoice ${id} by unauthorized user ${userId} (invoice belongs to ${anyInvoice.userId})`,
        );
        await queryRunner.rollbackTransaction();
        await queryRunner.release();
      ...
      if (!invoiceLocked) {
        ...
        if (recheckInvoice?.deletedAt) {
          ...
          this.logger.debug(
            `Invoice ${id} was soft-deleted concurrently, returning early (idempotent)`,
          );
          await queryRunner.rollbackTransaction();
          await queryRunner.release();
          return; // Idempotent - already deleted, success
        ...
        this.logger.warn(
          `Invoice ${id} not found with lock after initial check - possible race condition or data inconsistency`,
        );
        await queryRunner.rollbackTransaction();
        await queryRunner.release();
        throw new NotFoundException(`Invoice with ID ${id} not found`);
      ...
      if (invoice.items && invoice.items.length > 0) {
        ...
        this.logger.debug(
          `Deleted ${invoice.items.length} invoice items for invoice ${id}`,
        );
      }
      ...
      if (isActive) {
        try {
          ...
          this.logger.error(
            `Failed to rollback transaction during invoice deletion:`,
            rollbackError,
          );
        }
      }
      this.logger.error(
        `Invoice deletion failed for ${id}, transaction rolled back:`,
        error,
      );
      throw error;
    } finally {
  ...
  async convertEstimateToInvoice(id: string, userId: string): Promise<Invoice> {
    ...
    try {
      ...
      if (estimate.type !== 'estimate') {
        throw new BadRequestException(
          'Only estimates can be converted to invoices',
        );
      }
      ...
      const settings = await this.userSettingsService.getSettings(userId);
      estimate.number = await this.generateInvoiceNumber(
        userId,
        'invoice',
        settings,
        queryRunner,
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(
        `Estimate conversion failed for ${id}, transaction rolled back:`,
        error,
      );
      throw error;
    } finally {
  ...
  async duplicateInvoice(id: string, userId: string): Promise<Invoice> {
    ...
    const duplicateData: CreateInvoiceDto & {
      ...
      dueDate: originalInvoice.dueDate
        ? new Date(originalInvoice.dueDate).toISOString().split('T')[0]
        : undefined,
  ...
  async getStats(userId: string): Promise<{
    ...
    if (cached) {
      ...
      this.logger.log(
        `[DIAGNOSTIC] Returning cached stats: totalCount=${cached.totalCount}`,
      );
      return cached;
    }
    ...
    this.logger.log(
      `[DIAGNOSTIC] Raw stats from database: ${JSON.stringify(stats)}`,
    );
    ...
    this.logger.log(
      `[DIAGNOSTIC] Final stats result: ${JSON.stringify(result)}`,
    );
  ...
  async backfillPaidAtDates(userId: string) {
    ...
    for (const invoice of paidInvoicesWithoutPaidAt) {
      ...
      updatedCount++;
      this.logger.log(
        `Updated invoice ${invoice.number} with paidAt: ${paidAtDate}`,
      );
    }
  ...
  private async generateInvoiceNumber(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      this.logger.warn(
        `Invoice number ${invoiceNumber} already exists, retrying (attempt ${attempt + 1}/${maxRetries})`,
      );
    ...
    this.logger.warn(
      `Failed to generate unique invoice number after ${maxRetries} attempts, using fallback: ${fallbackNumber}`,
    );
  ...
  async sendEmail(
    ...
    try {
      ...
      if (invoiceToUpdate.status === 'draft') {
        ...
        if (
          invoiceToUpdate.type !== 'estimate' &&
          invoiceToUpdate.items &&
          invoiceToUpdate.items.length > 0
        ) {
          for (const item of invoiceToUpdate.items) {
            if (item.inventoryItemId) {
              ...
              if (!inventoryItem) {
                this.logger.warn(
                  `Inventory item ${item.inventoryItemId} not found, skipping stock movement`,
                );
                continue;
              }
      ...
      try {
        ...
        if (!recipientEmail) {
          this.logger.warn(
            `Invoice ${invoiceToUpdate.number} has no recipient email, skipping email send`,
          );
          return {
            message:
              'Invoice status updated to sent. Email not sent: recipient email not available.',
          };
        }

        const emailSubject =
          options?.subject ||
          `Invoice ${invoiceToUpdate.number} from ${invoiceToUpdate.store?.name || 'Your Company'}`;
        const emailMessage =
          options?.message || this.buildInvoiceEmailHtml(invoiceToUpdate);
        ...
        this.logger.log(
          `Invoice ${invoiceToUpdate.number} email sent successfully to ${recipientEmail}`,
        );
        ...
        this.logger.error(
          `Failed to send email for invoice ${invoiceToUpdate.number}:`,
          emailError,
        );
        // Status is already updated, so we return success but note the email failure
        return {
          message:
            'Invoice status updated to sent, but email failed to send. Please check email configuration.',
      ...
      await queryRunner.rollbackTransaction();
      this.logger.error(
        `Failed to update invoice status for email send:`,
        error,
      );
      throw error;
    } finally {
