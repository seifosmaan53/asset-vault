...
export class StoreService {
  ...
  async create(userId: string, createStoreDto: CreateStoreDto): Promise<Store> {
    ...
    if (!client) {
      throw new NotFoundException(
        `Client with ID "${createStoreDto.clientId}" not found`,
      );
    }

    const phoneTrimmed =
      typeof createStoreDto.phone === 'string'
        ? createStoreDto.phone.trim()
        : createStoreDto.phone;
    const emailTrimmed =
      typeof createStoreDto.email === 'string'
        ? createStoreDto.email.trim()
    ...
    const normalized = {
      ...
      email: emailTrimmed
        ? (sanitizeEmail(emailTrimmed) ?? undefined)
        : undefined,
      address: createStoreDto.address
        ? sanitizeString(createStoreDto.address, 500)
        : createStoreDto.address,
      city: createStoreDto.city
        ? sanitizeString(createStoreDto.city, 100)
        : createStoreDto.city,
      state: createStoreDto.state
        ? sanitizeString(createStoreDto.state, 100)
        : createStoreDto.state,
      zip: createStoreDto.zip
        ? sanitizeString(createStoreDto.zip, 30)
        : createStoreDto.zip,
      country: createStoreDto.country
        ? sanitizeString(createStoreDto.country, 100)
        : createStoreDto.country,
      notes: createStoreDto.notes
        ? sanitizeString(createStoreDto.notes, 5000)
        : createStoreDto.notes,
    ...
    try {
      ...
      this.logger.log(
        `Store created: ${savedStore.name} (${savedStore.code}) for user ${userId}`,
      );
      ...
      if (error.code === '23505') {
        ...
        if (
          constraintName.includes('UX_stores_org_code') ||
          constraintName.includes('UX_stores_user_code_legacy')
        ) {
          throw new ConflictException(
            `Store with code "${normalized.code}" already exists. ` +
  ...
  async update(
    ...
    if (updateStoreDto.clientId && updateStoreDto.clientId !== store.clientId) {
      ...
      if (!client) {
        throw new NotFoundException(
          `Client with ID "${updateStoreDto.clientId}" not found`,
        );
      }
    }
    ...
    if (normalizedUpdate.name !== undefined) {
      normalizedUpdate.name = normalizedUpdate.name
        ? this.normalizeName(normalizedUpdate.name)
        : normalizedUpdate.name;
    }
    if (normalizedUpdate.code !== undefined) {
      normalizedUpdate.code = normalizedUpdate.code
        ? this.normalizeCode(normalizedUpdate.code)
        : normalizedUpdate.code;
    }
    if (normalizedUpdate.phone !== undefined) {
      const phoneTrimmed =
        typeof normalizedUpdate.phone === 'string'
          ? normalizedUpdate.phone.trim()
          : normalizedUpdate.phone;
      normalizedUpdate.phone = phoneTrimmed
        ? sanitizeString(phoneTrimmed, 50)
        : undefined;
    }
    if (normalizedUpdate.email !== undefined) {
      const emailTrimmed =
        typeof normalizedUpdate.email === 'string'
          ? normalizedUpdate.email.trim()
          : normalizedUpdate.email;
      normalizedUpdate.email = emailTrimmed
        ? (sanitizeEmail(emailTrimmed) ?? undefined)
        : undefined;
    }
    if (normalizedUpdate.address !== undefined) {
      normalizedUpdate.address = normalizedUpdate.address
        ? sanitizeString(normalizedUpdate.address, 500)
        : normalizedUpdate.address;
    }
    if (normalizedUpdate.city !== undefined) {
      normalizedUpdate.city = normalizedUpdate.city
        ? sanitizeString(normalizedUpdate.city, 100)
        : normalizedUpdate.city;
    }
    if (normalizedUpdate.state !== undefined) {
      normalizedUpdate.state = normalizedUpdate.state
        ? sanitizeString(normalizedUpdate.state, 100)
        : normalizedUpdate.state;
    }
    if (normalizedUpdate.zip !== undefined) {
      normalizedUpdate.zip = normalizedUpdate.zip
        ? sanitizeString(normalizedUpdate.zip, 30)
        : normalizedUpdate.zip;
    }
    if (normalizedUpdate.country !== undefined) {
      normalizedUpdate.country = normalizedUpdate.country
        ? sanitizeString(normalizedUpdate.country, 100)
        : normalizedUpdate.country;
    }
    if (normalizedUpdate.notes !== undefined) {
      normalizedUpdate.notes = normalizedUpdate.notes
        ? sanitizeString(normalizedUpdate.notes, 5000)
    ...
    if (normalizedUpdate.code && normalizedUpdate.code !== store.code) {
      ...
      if (existingStore && existingStore.id !== id) {
        throw new ConflictException(
          `Store with code "${normalizedUpdate.code}" already exists`,
        );
      }
    }
    ...
    this.logger.log(
      `Store updated: ${updatedStore.name} (${updatedStore.code}) for user ${userId}`,
    );
  ...
  async remove(id: string, userId: string): Promise<void> {
    this.logger.debug(
      `remove: Starting deletion for store ${id}, userId=${userId}`,
    );
    ...
    try {
      ...
      if (anyStore.deletedAt) {
        await queryRunner.rollbackTransaction();
        this.logger.debug(
          `remove: Store ${id} is already soft-deleted (deletedAt=${anyStore.deletedAt}), returning early`,
        );
      ...
      if (!store) {
        ...
        await queryRunner.rollbackTransaction();
        this.logger.debug(
          `remove: Store ${id} not found with lock - may have been deleted concurrently`,
        );
      ...
      if (store.deletedAt) {
        await queryRunner.rollbackTransaction();
        this.logger.debug(
          `remove: Store ${id} is already soft-deleted (deletedAt=${store.deletedAt}), returning early`,
        );
        return; // Idempotent - already deleted, success
      }
      ...
      if (activeInvoices > 0) {
        await queryRunner.rollbackTransaction();
        this.logger.warn(
          `remove: Cannot delete store ${id}: has ${activeInvoices} active invoice(s)`,
        );
      ...
      if (itemsWithStock > 0) {
        await queryRunner.rollbackTransaction();
        this.logger.warn(
          `remove: Cannot delete store ${id}: has ${itemsWithStock} inventory item(s) with stock`,
        );
        throw new BadRequestException(
      ...
      if (updateResult.affected === 0) {
        this.logger.error(
          `remove: CRITICAL - UPDATE query affected 0 rows for store ${id}!`,
        );
        await queryRunner.rollbackTransaction();
        throw new Error(
          'Failed to soft-delete store: UPDATE query did not affect any rows',
        );
      }

      this.logger.debug(
        `remove: UPDATE query executed for store ${id}, affected rows: ${updateResult.affected}`,
      );
      ...
      if (!verifyDeleted) {
        this.logger.error(
          `remove: CRITICAL - Store ${id} not found after UPDATE!`,
        );
        await queryRunner.rollbackTransaction();
        throw new Error(
          'Failed to soft-delete store: store not found after deletion',
        );
      }

      if (!verifyDeleted.deletedAt) {
        this.logger.error(
          `remove: CRITICAL - Store ${id} was not actually soft-deleted! deletedAt is null after UPDATE`,
        );
        await queryRunner.rollbackTransaction();
      ...
      this.logger.debug(
        `remove: Verified store ${id} has deletedAt=${verifyDeleted.deletedAt} in database before commit`,
      );
      ...
      this.logger.debug(
        `remove: Store ${id} soft-deleted successfully using direct UPDATE query`,
      );
      ...
      if (error.code === '23503') {
        ...
        throw new NotFoundException(
          'Cannot delete store: has associated records',
        );
      }
