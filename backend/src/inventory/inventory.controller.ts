...
export class InventoryController {
  ...
  @ApiOperation({
    ...
    description:
      'Create multiple inventory items in a single transaction. Returns successful items and errors for failed items.',
  })
  @ApiResponse({
    status: 201,
    description:
      'Bulk creation completed. Check response for success and error details.',
  })
  @ApiResponse({
    status: 400,
    description: 'Bad request - validation failed or all items failed',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  async bulkCreate(
    @Body() bulkDto: BulkCreateInventoryItemsDto,
    @Request() req,
  ) {
    try {
      // Organizations removed - data is user-scoped
      const result = await this.inventoryService.bulkCreate(
        req.user.userId,
        bulkDto.items,
      );
      return result;
  ...
  @ApiOperation({
    ...
    description: 'Delete an inventory item (Admin/Owner only)',
  })
  @ApiParam({ name: 'id', description: 'Inventory item UUID' })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Retrieve all stock movements for a specific inventory item',
  })
  @ApiParam({ name: 'id', description: 'Inventory item UUID' })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Create a new stock movement (purchase, sale, or adjustment) for an item',
  })
  @ApiParam({ name: 'id', description: 'Inventory item UUID' })
  @ApiResponse({
    status: 201,
  ...
  createMovement(
    ...
    try {
      // Organizations removed - data is user-scoped
      const result = this.inventoryService.createMovement(
        id,
        req.user.userId,
        createDto,
      );
  ...
  @ApiOperation({
    summary: 'Get linked invoices',
    description: 'Retrieve all invoices that include this inventory item',
  })
  @ApiParam({ name: 'id', description: 'Inventory item UUID' })
  @ApiResponse({
    status: 200,
    description: 'Linked invoices retrieved successfully',
  })
  @ApiResponse({ status: 404, description: 'Inventory item not found' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  ...
  @ApiOperation({
    ...
    description:
      'Delete multiple inventory items at once. Returns count of deleted and failed deletions. Admin/Owner only.',
  })
  @ApiResponse({
    status: 200,
    description: 'Bulk deletion completed. Check response for details.',
  })
  @ApiResponse({ status: 400, description: 'Bad request - validation failed' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({
  ...
  async bulkDelete(
    @Body() bulkDeleteDto: BulkDeleteInventoryDto,
    @Request() req,
  ) {
    // Organizations removed - data is user-scoped
    const result = await this.inventoryService.bulkRemove(
      bulkDeleteDto.ids,
      req.user.userId,
    );
    return {
  ...
  @ApiOperation({
    summary: 'Import inventory items from CSV/Excel',
    description:
      'Import multiple inventory items from a CSV or Excel file. Returns count of created and failed imports.',
  })
  @ApiResponse({
    status: 200,
    description: 'Import completed. Check response for details.',
  })
  @ApiResponse({
    status: 400,
  ...
  async import(@UploadedFile() file: Express.Multer.File, @Request() req) {
    ...
    // Organizations removed - data is user-scoped
    const result = await this.inventoryService.importFromFile(
      file,
      req.user.userId,
    );
    return {
