...
const InvoicesList = () => {
  ...
  /* The type filter (invoice vs estimate) was removed when estimates went away. The
     state was kept "for backward compatibility" but nothing has read it since, and it was
     always 'all'. */
  ...
  /* `isDataLoading = isLoading || isRefetching` was computed here and never read — the
     loading state below uses isLoading directly, so a background refetch was never meant
     to blank the table. */
          ...
          async (invoice: Invoice) => {
            ...
            try {
              const restoreData: CreateInvoiceDto = {
                ...
                type: invoice.type,
                // Invoice.issueDate and .dueDate are strings — the API returns JSON — so
                // the instanceof Date and typeof branches that stood here could never be
                // taken.
