  ...
  const invoiceFilters = useMemo(() => {
    ...
    if (advancedFilters) {
      ...
      advancedFilters.dateRanges.forEach(range => {
        if (range.startDate) {
          const fieldMap: Record<string, 'issueDateFrom' | 'dueDateFrom' | 'paidDateFrom' | 'createdAtFrom'> = {
        ...
        if (range.endDate) {
          const fieldMap: Record<string, 'issueDateTo' | 'dueDateTo' | 'paidDateTo' | 'createdAtTo'> = {
      ...
      advancedFilters.amountRanges.forEach(range => {
        if (range.min !== null) {
          const fieldMap: Record<string, 'totalMin' | 'subtotalMin'> = {
            'Total': 'totalMin',
        ...
        if (range.max !== null) {
          const fieldMap: Record<string, 'totalMax' | 'subtotalMax'> = {
            'Total': 'totalMax',
