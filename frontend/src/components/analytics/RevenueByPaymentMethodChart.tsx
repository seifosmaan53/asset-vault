...
import { useRevenueByPaymentMethod } from '../../hooks/useAnalytics';
...
const RevenueByPaymentMethodChart = ({ startDate, endDate, storeId }: RevenueByPaymentMethodChartProps) => {
  ...
  if (isLoading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 200, minHeight: 200 }}>
        <CircularProgress />
      </Box>
    );
  }
  ...
  if (error) {
    return (
      ...
      >
        <PaymentIcon sx={{ fontSize: 40, color: 'error.main', mb: 1, opacity: 0.5 }} />
  ...
  if (!data || data.length === 0) {
    return (
      ...
      >
        <PaymentIcon sx={{ fontSize: 40, color: 'text.secondary', mb: 1, opacity: 0.5 }} />
  ...
  const chartHeight = Math.max(200, Math.min(350, chartData.length * 40 + 60));

  return (
    <Box sx={{ width: '100%', height: `${chartHeight}px`, minWidth: 0, minHeight: '200px' }}>
